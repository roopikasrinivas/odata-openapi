...
module.exports.csdl2openapi = function (
    ...
    function optionSelect(parameters, element, target, restrictions) {
        ...
        if (selectSupport.Supported !== false) {
            const items = selectItems(element.$Type);
            if (items.length > 0) {
                parameters.push({
    ...
    /**
     * Construct select items for structured type
     * @param {object} typename Type name
     * @param {string} prefix Prefix for select items
     * @return {Array} List of select items
     */
    function selectItems(typename, prefix = '') {
        const type = modelElement(typename) || {};
        const items = [];

        Object.keys(type)
            .filter(key => isIdentifier(key) && type[key].$Kind != 'NavigationProperty')
            .forEach(key => {
                items.push(prefix + key);
                //TODO: recursive complex types
                if (type[key].$Type)
                    items.push(...selectItems(type[key].$Type, prefix + key + '/'));
            });
    ...
    function optionExpand(parameters, element, target) {
        ...
        if (supported) {
            const items = ['*']
            items.push(...expandItems(element.$Type));

            //TODO: ExpandRestrictions/NonExpandableProperties (needed from source/root)
    ...
    /**
     * Construct expand items for structured type
     * @param {object} typename Type name
     * @param {string} prefix Prefix for expand items
     * @param {integer} level Number of navigation segments so far
     * @return {Array} List of expand items
     */
    function expandItems(typename, prefix = '', visitedTypes = [], level = 0) {
        const type = modelElement(typename) || {};
        const items = [];
        ...
        Object.keys(type)
            .filter(key => isIdentifier(key) && type[key].$Kind == 'NavigationProperty')
            .forEach(key => {
                if (level < maxLevels && !(visitedTypes.includes(type[key].$Type))) {
                    items.push(prefix + key);
                    items.push(...expandItems(type[key].$Type, prefix + key + '/', visitedTypes.concat([typename]), level + 1));
